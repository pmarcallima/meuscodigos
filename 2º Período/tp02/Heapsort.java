...
//Metodo que constroi o heap original
   public void construir(int tamHeap){
      for(int i = tamHeap; i > 1 && compare2(array[i], array[i/2]); i /= 2){
         swap(i, i/2);
	 mov+=2;
...
//Metodo para reconstruir o heap após a retirada de um elemento
   public void reconstruir(int tamHeap){
      int i = 1;
      while(i <= (tamHeap/2)){
         int filho = getMaiorFilho(i, tamHeap);
         if(!compare2(array[i], array[filho])){
            swap(i, filho);
	    mov+=2;
            i = filho;
         }else{
...
//Metodo para comparar e retornar a posição do maior filho de um pai
   public int getMaiorFilho(int i, int tamHeap){
      int filho;
      if (2*i == tamHeap || (compare2(array[2*i], array[2*i+1]))){
         filho = 2*i;
      } else {
         filho = 2*i + 1;
      }
...
//Metodo que constrói o arquivo log da ordenação por seleção
   public void log()
   {
	   Arq.openWrite("815336_heapsort.txt");
	   Arq.print("815336\t");
	   Arq.print(cmp);
	   Arq.print("\t");
	   Arq.print(mov);
