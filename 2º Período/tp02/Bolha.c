...
//metodo para comparar jogadores com o atributo primário sendo o anoNascimento e o secundário o nome
...
//metodo de ordenação por bolha
void bolha(Jogador *array, Log*log, int n){
    int i, j;
    for (i = 0; i < (n-1); i++) {
      for (j = 0; j < (n-1); j++) {
         if (compare2(log, &array[j], &array[j + 1] )> 0) {
...
{
 FILE* arq = fopen("815336_Bolha.txt", "w");
  if(arq == NULL)
