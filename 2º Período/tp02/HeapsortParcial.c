...
//Metodo para contruir o Heap original
void construir(Jogador *arrayTmp, Log *log, int tamHeap){
    for(int i = tamHeap; i > 1 && (compare2(log,&arrayTmp[i],&arrayTmp[i/2])>0); i /= 2){
	log->mov+=2;
        swap(&arrayTmp[i], &arrayTmp[i/2]);
    }
}
//Metodo para comparar os 2 filhos de um pai e dizer qual o maior
int getMaiorFilho(Jogador *arrayTmp, Log *log, int i, int tamHeap){
    int filho;
    if (2*i == tamHeap || (compare2(log,&arrayTmp[2*i], &arrayTmp[2*i+1])>0)){
        filho = 2*i;
    } else {
        filho = 2*i + 1;
    }
...
//Metodo de reconstruçao do Heap
void reconstruir(Jogador *arrayTmp, Log *log, int tamHeap){
    int i = 1;
    while(i <= (tamHeap/2)){
        int filho = getMaiorFilho(arrayTmp, log, i, tamHeap);
        if(compare2(log, &arrayTmp[i], &arrayTmp[filho])==0){
	    log->mov +=2;	
            swap(&arrayTmp[i], &arrayTmp[filho]);
            i = filho;
        }else{
            i = tamHeap;
        }
...
//Metodo de Heapsort
void heapsort(Jogador *array, Log *log, int n) {
    ...
    for(int tamHeap = 2; tamHeap <= 10; tamHeap++){
        construir(arrayTmp, log, tamHeap);
    }
    ...
    {
	if(compare2(log, &arrayTmp[i], &arrayTmp[1])==0)
	{
        log->mov +=2;
        swap(&arrayTmp[i], &arrayTmp[1]);
      reconstruir(arrayTmp, log, 10);
	}
    }
    tamHeap = 10;
    while(tamHeap >1)
    {
	    ...
	    reconstruir(arrayTmp, log, tamHeap);
    }
...
{
 FILE* arq = fopen("815336_heapsortParcial.txt", "w");
  if(arq == NULL)
