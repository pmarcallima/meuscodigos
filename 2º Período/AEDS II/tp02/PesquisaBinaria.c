...
//metodo de ordenação por seleçao baseado no nome
...
//metodo de inicialização da recursividade no seleção
void sort(Jogador *array, int n)
{
	selecao(array, n, 0);
}
//metodo de pesquisa binária com base no atributo nome
int pesqBin(Jogador *array, Log *log, int n, char *x){
      bool resp = false;
...
{
 FILE* arq = fopen("815336_PesquisaBinaria.txt", "w");
  if(arq == NULL)
	  printf("Arquivo nao encontrado");
  else
  {
	  fprintf(arq, "815336\t");
	  fprintf(arq, "%d\t", log->cmp);
...
start_t = clock();
  while(1)
  {
   ...
   bool resp = pesqBin(z,log, i, temp);
   if(resp == true)
   {
	   printf("SIM\n");
