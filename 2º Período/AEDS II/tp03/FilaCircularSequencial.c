...
{
	int j = 0;	
	for(int i = f->primeiro; i!=f->ultimo; i = ((i +1) % MAXTAM))
	{
	printf("[");
	printf("%d]", j);
	printf(" ## ");
	printf("%s", getNome(&f->array[i]));
	printf(" ## ");
	printf("%d", getAltura(&f->array[i]));
	printf(" ## ");
	printf("%d", getPeso(&f->array[i]));
	printf(" ## ");
	printf("%d", getAnoNascimento(&f->array[i]));
	printf(" ## "); 
	printf("%s", getUniversidade(&f->array[i]));
	printf(" ## "); 
	printf("%s", getCidadeNascimento(&f->array[i]));
	printf(" ## ");
	printf("%s ##", getEstadoNascimento(&f->array[i]));
	printf("\n");
	j++;
	}
}
...
Jogador removerInicio(Fila *f)
{
       ...
       Jogador removido = f->array[f->primeiro];
